...
### ステップ1a（「加入したい」「検討したい」など加入意向を受けたとき） — **必ずここで止まる**
- 車種区分・車齢が**まだわかっていない**場合:
  1. 以下をまとめて1メッセージで顧客に質問する：
     - 車種区分（普通車 / 軽自動車 / SUV など）
     - 車齢（新車・購入予定、または年数）
     - 希望プランや予算感（あれば）
  2. **ここで応答を完了し、顧客の返答を待つ。`recommend.py` も `quote.py` も絶対に実行しない。**

### ステップ1b（顧客から車種・車齢が得られたとき） — **ここで止まる**
- 車種区分・車齢が**揃っている**場合のみ実行:
  1. `auto-insurance-recommendation` スキルを使ってプラン候補を提示する
  2. **ここで応答を完了し、顧客のプラン選択を待つ。`quote.py` も `create_contract.py` も実行しない。**
